...
def pallas_call_tpu_lowering_rule(
  ...
  if jax_mesh is not None:
    ...
    if mesh_axes:
      if metadata is None:
        metadata = frozen_dict.FrozenDict(dict(mesh_axes=sorted(mesh_axes)))
      else:
        if "mesh_axes" in metadata:
          raise ValueError("Metadata already contains mesh axes.")
        updated = dict(metadata, mesh_axes=sorted(mesh_axes))
        metadata = frozen_dict.FrozenDict(updated)
